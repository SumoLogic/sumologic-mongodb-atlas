...
class FetchMixin(MongoDBAPI):
    def fetch(self):
        ...
        try:
            ...
            self.log.exception(
                f"""Exception {e} occurred while fetching LogType: {log_type}"""
            )
        finally:
            if output_handler:
...
class PaginatedFetchMixin(MongoDBAPI):
    def fetch(self):
        ...
        try:
            ...
            self.log.info(
                f"""Fetching LogType: {log_type}  starttime: {kwargs['params']['minDate']} endtime: {kwargs['params']['maxDate']}"""
            )
            while next_request:
