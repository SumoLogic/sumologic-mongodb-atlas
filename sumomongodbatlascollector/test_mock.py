...
def test_mongodb_api(mock_base_api):
    ...
    api = MongoDBAPI(Mock(), mock_config)
    ...
    assert api._replace_cluster_name("test-cluster-shard-0", {"test-cluster": "alias"}) == "alias-shard-0"
...
def test_fetch_mixin(mock_client_mixin, mock_output_handler_factory):
    ...
    fetch_mixin.transform_data = Mock(return_value=([{'transformed': 'data'}], {'state': 'updated'}))
...
def test_paginated_fetch_mixin(mock_client_mixin, mock_output_handler_factory):
    ...
    paginated_fetch_mixin.build_fetch_params = Mock(return_value=('http://test.com', {'params': {'pageNum': 1, 'minDate': '2023-01-01', 'maxDate': '2023-01-02'}}))
    paginated_fetch_mixin.transform_data = Mock(return_value=([{'transformed': 'data'}], {'state': 'updated'}))
...
def test_log_api(mock_client_mixin, mock_output_handler_factory, mock_base_api):
    ...
    log_api = LogAPI(mock_kvstore, 'test-hostname', 'mongodb.log', mock_config, mock_cluster_mapping)
    
    assert log_api.get_key() == 'test_project-test-hostname-mongodb.log'
    
    log_api.save_state(1000000)
    mock_kvstore.set.assert_called_with('test_project-test-hostname-mongodb.log', {'last_time_epoch': 1000000})
    
    mock_kvstore.has_key.return_value = False
    log_api.get_state()
    mock_kvstore.set.assert_called_with('test_project-test-hostname-mongodb.log', {'last_time_epoch': log_api.DEFAULT_START_TIME_EPOCH})
    
    log_api.get_window = Mock(return_value=(1000000, 2000000))
    url, params = log_api.build_fetch_params()
    assert url == 'https://api.mongodb.com/groups/test_project/clusters/test-hostname/logs/mongodb.log'
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.get_current_timestamp', return_value=1600000):
        result, state = log_api.check_move_fetch_window({'params': {'endDate': 1500000}})
        assert result == True
        assert state == {'last_time_epoch': 1500000}
    
    sample_log = {
        't': {'$date': '2023-07-26T12:00:00.000Z'},
...
@patch('sumoappclient.sumoclient.factory.OutputHandlerFactory')
@patch('sumoappclient.sumoclient.httputils.ClientMixin')
def test_process_metrics_api(mock_client_mixin, mock_output_handler_factory, mock_base_api):
    mock_kvstore = Mock()
    mock_config = {
        'MongoDBAtlas': {
            'PROJECT_ID': 'test_project',
            ...
            'METRIC_TYPES': {
                'PROCESS_METRICS': ['metric1', 'metric2']
            }
        }
    }
    mock_cluster_mapping = {'test-cluster': 'alias'}
    
    process_metrics_api = ProcessMetricsAPI(mock_kvstore, 'test-process-id', mock_config, mock_cluster_mapping)
    
    assert process_metrics_api.get_key() == 'test_project-test-process-id'
    
    process_metrics_api.save_state(1000000)
    mock_kvstore.set.assert_called_with('test_project-test-process-id', {'last_time_epoch': 1000000})
    
    mock_kvstore.has_key.return_value = False
    process_metrics_api.get_state()
    mock_kvstore.set.assert_called_with('test_project-test-process-id', {'last_time_epoch': process_metrics_api.DEFAULT_START_TIME_EPOCH})
    
    with patch('sumoappclient.sumoclient.mongodbapi.convert_epoch_to_utc_date', side_effect=['2023-07-26T00:00:00Z', '2023-07-26T01:00:00Z']):
        process_metrics_api.get_window = Mock(return_value=(1000000, 2000000))
        url, params = process_metrics_api.build_fetch_params()
        assert url == 'https://api.mongodb.com/groups/test_project/processes/test-process-id/measurements'
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.get_current_timestamp', return_value=1600000):
        with patch('sumoappclient.sumoclient.mongodbapi.convert_utc_date_to_epoch', return_value=1500000):
            result, state = process_metrics_api.check_move_fetch_window({'params': {'end': '2023-07-26T01:00:00Z'}})
            assert result == True
            assert state == {'last_time_epoch': 1500000}
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.convert_utc_date_to_epoch', return_value=1627257600):
...
m unittest.mock import Mock, patch
import json

@patch('sumoappclient.sumoclient.base.BaseAPI')
@patch('sumoappclient.sumoclient.factory.OutputHandlerFactory')
@patch('sumoappclient.sumoclient.httputils.ClientMixin')
def test_disk_metrics_api(mock_client_mixin, mock_output_handler_factory, mock_base_api):
    mock_kvstore = Mock()
    mock_config = {
        'MongoDBAtlas': {
            'PROJECT_ID': 'test_project',
            ...
            'PAGINATION_LIMIT': 100,
            'METRIC_TYPES': {
                'DISK_METRICS': ['disk_metric1', 'disk_metric2']
            }
        }
    }
    mock_cluster_mapping = {'test-cluster': 'alias'}
    
    disk_metrics_api = DiskMetricsAPI(mock_kvstore, 'test-process-id', 'test-disk', mock_config, mock_cluster_mapping)
    
    assert disk_metrics_api.get_key() == 'test_project-test-process-id-test-disk'
    
    disk_metrics_api.save_state(1000000)
    mock_kvstore.set.assert_called_with('test_project-test-process-id-test-disk', {'last_time_epoch': 1000000})
    
    mock_kvstore.has_key.return_value = False
    disk_metrics_api.get_state()
    mock_kvstore.set.assert_called_with('test_project-test-process-id-test-disk', {'last_time_epoch': disk_metrics_api.DEFAULT_START_TIME_EPOCH})
    
    with patch('sumoappclient.sumoclient.mongodbapi.convert_epoch_to_utc_date', side_effect=['2023-07-26T00:00:00.000Z', '2023-07-26T01:00:00.000Z']):
        disk_metrics_api.get_window = Mock(return_value=(1000000, 2000000))
        url, params = disk_metrics_api.build_fetch_params()
        assert url == 'https://api.mongodb.com/groups/test_project/processes/test-process-id/disks/test-disk/measurements'
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.get_current_timestamp', return_value=1600000):
        with patch('sumoappclient.sumoclient.mongodbapi.convert_utc_date_to_epoch', return_value=1500000):
            result, state = disk_metrics_api.check_move_fetch_window({'params': {'end': '2023-07-26T01:00:00.000Z'}})
            assert result == True
            assert state == {'last_time_epoch': 1500000}
...
@patch('sumoappclient.sumoclient.factory.OutputHandlerFactory')
@patch('sumoappclient.sumoclient.httputils.ClientMixin')
def test_database_metrics_api(mock_client_mixin, mock_output_handler_factory, mock_base_api):
    mock_kvstore = Mock()
    mock_config = {
        'MongoDBAtlas': {
            'PROJECT_ID': 'test_project',
            ...
            'PAGINATION_LIMIT': 100,
            'METRIC_TYPES': {
                'DATABASE_METRICS': ['db_metric1', 'db_metric2']
            }
        }
    }
    mock_cluster_mapping = {'test-cluster': 'alias'}
    
    db_metrics_api = DatabaseMetricsAPI(mock_kvstore, 'test-process-id', 'test-database', mock_config, mock_cluster_mapping)
    
    assert db_metrics_api.get_key() == 'test_project-test-process-id-test-database'
    
    db_metrics_api.save_state(1000000)
    mock_kvstore.set.assert_called_with('test_project-test-process-id-test-database', {'last_time_epoch': 1000000})
    
    mock_kvstore.has_key.return_value = False
    db_metrics_api.get_state()
    mock_kvstore.set.assert_called_with('test_project-test-process-id-test-database', {'last_time_epoch': db_metrics_api.DEFAULT_START_TIME_EPOCH})
    
    with patch('sumoappclient.sumoclient.mongodbapi.convert_epoch_to_utc_date', side_effect=['2023-07-26T00:00:00.000Z', '2023-07-26T01:00:00.000Z']):
        db_metrics_api.get_window = Mock(return_value=(1000000, 2000000))
        url, params = db_metrics_api.build_fetch_params()
        assert url == 'https://api.mongodb.com/groups/test_project/processes/test-process-id/databases/test-database/measurements'
        assert params['params']['start'] == '2023-07-26T00:00:00.000Z'
        assert params['params']['end'] == '2023-07-26T01:00:00.000Z'
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.get_current_timestamp', return_value=1600000):
        with patch('sumoappclient.sumoclient.mongodbapi.convert_utc_date_to_epoch', return_value=1500000):
            result, state = db_metrics_api.check_move_fetch_window({'params': {'end': '2023-07-26T01:00:00.000Z'}})
            assert result == True
            assert state == {'last_time_epoch': 1500000}
...
@patch('sumoappclient.sumoclient.factory.OutputHandlerFactory')
@patch('sumoappclient.sumoclient.httputils.ClientMixin')
def test_project_events_api(mock_client_mixin, mock_output_handler_factory, mock_base_api):
    mock_kvstore = Mock()
    mock_config = {
        'MongoDBAtlas': {
            'PROJECT_ID': 'test_project',
    ...
    mock_kvstore.set.assert_called_with('test_project-projectevents', {'last_time_epoch': 1000000, 'page_num': 1})
    
    mock_kvstore.has_key.return_value = False
    state = project_events_api.get_state()
    mock_kvstore.set.assert_called_with('test_project-projectevents', {'last_time_epoch': project_events_api.DEFAULT_START_TIME_EPOCH, 'page_num': 0})
    
    with patch('sumoappclient.sumoclient.mongodbapi.convert_epoch_to_utc_date', side_effect=['2023-07-26T00:00:00.000Z', '2023-07-26T01:00:00.000Z']):
        project_events_api.get_window = Mock(return_value=(1000000, 2000000))
        url, params = project_events_api.build_fetch_params()
        assert url == 'https://api.mongodb.com/groups/test_project/events'
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.get_current_timestamp', return_value=1600000):
        with patch('sumoappclient.sumoclient.mongodbapi.convert_utc_date_to_epoch', return_value=1500000):
            result, state = project_events_api.check_move_fetch_window({'params': {'maxDate': '2023-07-26T01:00:00.000Z'}})
            assert result == True
    ...
    with patch('sumoappclient.sumoclient.mongodbapi.convert_date_to_epoch', return_value=1627257600):
